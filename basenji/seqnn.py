...
class SeqNN(nn.Module):
    def __init__(self):
        ...
        self.conv_block1 = ConvBlock(64, 64, kernel_size=15, stride=1, padding=7)
        self.conv_block2 = ConvBlock(64, 72, kernel_size=15, stride=1, padding=7)
        self.dropout = ConvConvDropBlock(dilation=1)
        self.dropout1 = ConvConvDropBlock(dilation=2)
        self.dropout2 = ConvConvDropBlock(dilation=4)
        self.dropout3 = ConvConvDropBlock(dilation=8)
        self.dropout4 = ConvConvDropBlock(dilation=16)
        self.dropout5 = ConvConvDropBlock(dilation=32)
    ...
    def forward(self, x):
        ...
        batch_normalization = self.conv_block1(max_pooling1d)
        max_pooling1d_1 = F.max_pool1d(batch_normalization, kernel_size=4)
        batch_normalization = self.conv_block2(max_pooling1d_1)
        max_pooling1d_2 = F.max_pool1d(batch_normalization, kernel_size=4)
        dropout = self.dropout(max_pooling1d_2)
        add = max_pooling1d_2 + dropout
        dropout_1 = self.dropout1(add)
        add_1 = add + dropout_1
        dropout_2 = self.dropout2(add_1)
        add_2 = add_1 + dropout_2
        dropout_3 = self.dropout3(add_2)
        add_3 = add_2 + dropout_3
        dropout_4 = self.dropout4(add_3)
        add_4 = add_3 + dropout_4
        dropout_5 = self.dropout5(add_4)
        add_5 = add_4 + dropout_5
        batch_normalization = self.conv_block3(add_5)
        dropout_6 = F.dropout(batch_normalization, p=0.05)
        gelu_16 = F.gelu(dropout_6)
        gelu_16 = gelu_16.view(-1, 64)
