...
def main():
    ...
    # 在输出路径上添加上当前的时间
    options.out_dir = options.out_dir +"pt" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    ...
    # 如果训练序列的步长小于一，设置新的训练步长为序列长度*训练步长
    ...
    # 读取目标数据集，此处为data/hear_wigs.txt
    # read target datasets
    targets_df = pd.read_csv(targets_file, index_col=0, sep='\t')
    
    ################################################################
    # define genomic contigs 定义contigs
    ################################################################
    ...
    ################################################################
    # divide between train/valid/test 分割训练集、交叉检验集、测试集
    ################################################################
    # 如果设置了folds(-f)
    # label folds
    ...
    pt_dir = '%s/ptrecords' % options.out_dir
    ...
    for fold_set in fold_labels:
        ...
        while tfr_start <= fold_set_end:
            tfr_stem = '%s/%s-%d' % (pt_dir, fold_set, tfr_i)
            
            tfr_file = '%s.pt' % tfr_stem
...
def break_large_contigs(contigs, break_t, verbose=False):
    ...
    # 建立小根堆，此处的key是-ctg_len，value是contig
...
def divide_contigs_pct(contigs, test_pct, valid_pct, pct_abstain=0.2):
    ...
    for ctg_len, ctg in length_contigs:
        ...
        # 如果当前长度的contig大于避免率*测试缺口，则把测试集缺口设为0，同理修改交叉检验集
        # skip if too large
